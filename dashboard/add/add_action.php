...
function ic_add_new_action(){
	...
	if (isset($_POST['ic_add_new_submit'])){
		...
		if($type == 'html'){
			...
			$html_content_1 = stripslashes($_POST["html_content_1"]);
			$html_content_2 = stripslashes($_POST["html_content_2"]);
			
			if(strlen($html_content_1) > 0){
				$add_row = $wpdb->insert( 
				$table_name, 
				array(
					'name' => $name, 
					'type' => $type,
					'content_1' => $html_content_1,
					'content_2' => $html_content_2,
					'activeness' => $activeness,
					'shortcode' => 'yes',
				) 
				);
